...
Run these from the repo root. The desktop app can pick local `.vera` archives or folders, inspect and validate archives, search documents or corpora with context and visual grounding, convert PDFs with parser/model settings, view embedded source PDFs, preview page text and figures, and export the embedded source document through the sidecar protocol. The unpacked Windows build writes `VERA.exe` under `packages/vera-app/release/win-unpacked`. See [docs/desktop-app-architecture.md](docs/desktop-app-architecture.md) for the app architecture and next steps.
