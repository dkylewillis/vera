...
vera convert manual.pdf --model all-MiniLM-L6-v2
```
...
1. **Parse.** The PDF parser extracts text and layout from every page,
...
During conversion, the parser decomposes each page into normalized
...
- **Embedding** — two built-in models selected with `--model`: `hashing`
  ...
  `sentence-transformers/<id>` (local neural embeddings via the `ml` extra).

```bash
vera convert manual.pdf --model hashing               # default: offline, deterministic
vera convert manual.pdf --model all-MiniLM-L6-v2      # local neural model
```
...
- **Embedding provider plugins** (`vera.embedders` entry-point group) —
  ...
  UIs, and declared credential environment variables for hosted providers.
- **Strict resolution** — unknown pipeline or model names are rejected with
  an error instead of falling back to a built-in.
...
| [`vera-ingest`](https://pypi.org/project/vera-ingest/) | `import vera_ingest` | Conversion: PDF parsing (PyMuPDF/pdfplumber), table extraction, selective OCR, chunking, atomic archive writing, and viewer helpers for pages, figures, and regions |
...
In v0.3, PDF parsing moves out of `vera-ingest` into `vera-ingest-pymupdf`
and `vera-ingest-docling` plugin packages, leaving `vera-ingest` as the
provider-neutral registry and conversion orchestrator.
...
- [Architecture and contributing](docs/architecture.md) · [Troubleshooting](docs/troubleshooting.md)
